...
*** Compilation
First, all sources under =src/atmega328= must be compiled to an =.elf=. Instructions for compiling with the Microchip Studio IDE (which comes bundled with the AVR GCC toolchain) are given below;
...
If the microcontroller has a bootloader, it can be programmed over UART. A USB to UART bridge will be needed for this. If an Arduino is available, its USB to UART bridge may be used in place of a standalone bridge. Instruction for this will be given below;

- On the Arduino, patch the reset socket to one of the GND sockets. This will hold the Arduino's microcontroller in reset and prevent it from interfering with the UART link
...
From a PC on the LAN, ping the control module address to verify the network connection. If the ping is successful, issue an HTTP GET request and verify that the results are reasonable;
